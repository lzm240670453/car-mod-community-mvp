import { getKnowledgeCategory, listKnowledgeCategories, searchKnowledge } from "../../api/index";
import { KnowledgeArticle, KnowledgeCategory } from "../../types/index";
...
function mapCategory(item: KnowledgeCategory): KnowledgeCategoryItem {
...
Page({
  data: {
    ...
    breadcrumb: [] as KnowledgeCategory[],
    categories: [] as KnowledgeCategoryItem[],
  ...
  openArticle(event: WechatMiniprogram.TouchEvent) {
    const id = Number(event.currentTarget.dataset.id);
  ...
  goBreadcrumb(event: WechatMiniprogram.TouchEvent) {
    const id = Number(event.currentTarget.dataset.id);
  ...
  async load() {
    ...
    try {
      if (this.data.categoryId > 0) {
        const detail = await getKnowledgeCategory(this.data.categoryId);
        this.setData({
          title: detail.name,
